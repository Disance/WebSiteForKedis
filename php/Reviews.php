...
<?php
    ...
    ?>
        ...
        <select id="sortings" name="sorting" class="sorting">
                <option hidden selected disabled="disabled">Сортировка</option>
                <option value="id">Сортировка по давности</option>
                <option value="grade">Сортировка по Оценки</option>
            </select>
            <input id="button-sort" value="Сортировать" class="button-sort" type="submit">
        </form>
</div><br><br>
<div class="container">
        <div class="reviews">
        ...
        while($review = mysqli_fetch_assoc($result)){
            ...
            <h3><?php  $i=0; while( $i<$review['Grade']){ $i++;?> <svg style="float:right;" width="20px" height="20px" viewBox="0 0 24 24" fill="#CDA434" xmlns="http://www.w3.org/2000/svg">
                ...
                </svg><?php  } ?> </h3><br> <p style="color: blue;"><?php echo $carbon->diffForHumans($review['date']) ?></p>
